...
pub fn run_gui() -> wry::Result<()> {
    ...
    // let icon = include_bytes!("../img/wordle_finder_icon.ico").to_vec();
    // #[cfg(not(target_os = "windows"))]
    let icon: Vec<u8> = include_bytes!("../img/wordle_finder_icon.png").to_vec();
    let imagebuffer = RgbaImage::from_raw(256, 256, icon).unwrap();
    let pixels = imagebuffer.to_vec();
    // let pixels: Vec<u8> = imagebuffer.enumerate_pixels().iter().collect();
    ...
    let window = WindowBuilder::new()
        ...
        .with_window_icon(Some(Icon::from_rgba(pixels, 256, 256).unwrap()))
        .build(&event_loop)?;
